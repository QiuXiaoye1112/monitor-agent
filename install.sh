...
case $os_type in
    ...
    MINGW*|MSYS*|CYGWIN*)
        os_name="windows"
        target_dir="/c/monitor-agent"  # Use C:\monitor-agent on Windows
        ;;
    *)
...
agent_path="${target_dir}/monitor-agent"
...
echo -e "${WHITE}===========================================${NC}"
echo -e "${WHITE}    Monitor Agent Installation Script    ${NC}"
echo -e "${WHITE}===========================================${NC}"
echo ""
...
uninstall_previous() {
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files | grep -q "${service_name}.service"; then
        ...
        system_plist="/Library/LaunchDaemons/com.monitor.${service_name}.plist"
        user_plist="$HOME/Library/LaunchAgents/com.monitor.${service_name}.plist"
...
file_name="monitor-agent-${os_name}-${arch}"
...
if [ -n "$github_proxy" ]; then
    ...
    download_url="${github_proxy}/https://github.com/QiuXiaoye1112/monitor-agent/releases/${download_path}/${file_name}"
else
    # Direct access to GitHub releases
    download_url="https://github.com/QiuXiaoye1112/monitor-agent/releases/${download_path}/${file_name}"
fi
...
chmod +x "$agent_path"
log_success "Monitor Agent installed to ${GREEN}$agent_path${NC}"
...
if [ "$init_system" = "nixos" ]; then
    ...
    echo -e "${CYAN}  description = \"Monitor Agent Service\";${NC}"
...
EOF
    ...
    if [[ "$target_dir" =~ ^/Users/.* ]] || [ "$EUID" -ne 0 ]; then
        ...
        plist_file="$plist_dir/com.monitor.${service_name}.plist"
...
<dict>
    <key>Label</key>
    <string>com.monitor.${service_name}</string>
...
echo -e "${WHITE}===========================================${NC}"
if [ -f /etc/NIXOS ]; then
    log_success "Monitor Agent binary installed!"
    ...
    log_success "Monitor Agent installation completed!"
fi
