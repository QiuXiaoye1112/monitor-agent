...
for GOOS in "${OS_LIST[@]}"; do
  for GOARCH in "${ARCH_LIST[@]}"; do
    ...
    # 设置输出二进制文件名
    BINARY_NAME="monitor-agent-${GOOS}-${GOARCH}"
    if [ "$GOOS" = "windows" ]; then
      BINARY_NAME="${BINARY_NAME}.exe"
    fi

    # 构建二进制文件
    env GOOS=$GOOS GOARCH=$GOARCH CGO_ENABLED=0 go build -trimpath -ldflags="-X monitor-agent/update.CurrentVersion=${VERSION}" -o "./build/$BINARY_NAME"
